...
import { saveTelegramToken, detectTelegramChat, disconnectTelegram } from "@/lib/actions/settings";
...
export function TelegramPanel({
  ...
  async function handleSaveToken(e: React.FormEvent) {
    ...
    try {
      await saveTelegramToken(formData);
      setSaved(true);
      setToken("");
    } catch (err) {
  ...
  return (
    <Card>
      <h2 className="mb-1 text-sm font-semibold text-white/70">Telegram notifications</h2>
      <p className="mb-4 text-xs text-white/40">
        Get a Telegram message the instant a new order comes in — no need to keep the dashboard open.
        Uses your own bot, created for free via{" "}
      ...
      {isConnected ? (
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            onClick={() => startDisconnecting(() => disconnectTelegram())}
          >
            Disconnect
          </Button>
        </div>
        ...
        <div className="space-y-4">
          <form onSubmit={handleSaveToken} className="space-y-2">
            <Label htmlFor="telegram_bot_token">
              {hasToken ? "Bot token (saved — enter a new one to replace it)" : "1. Paste your bot token"}
            </Label>
          ...
          {hasToken && (
            <div className="space-y-2 border-t border-white/10 pt-4">
              <p className="text-xs text-white/60">
                ...
                and press <strong>Start</strong> (or send it any message), then click below.
              </p>
